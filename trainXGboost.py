...
class TrainXGBBoost:
    ...
    #    assert os.path.exists('./models/checkpoint')
        gan = GAN(num_features=48, num_historical_days=num_historical_days,
    ...
    #          saver.restore(sess, "{}models/{}".format(googlepath, model_name.decode()))
         ...
         #   files = [os.path.join('./stock_data', f) for f in os.listdir('./stock_data')]
            #files = [f"stock_data/{f}" for f in os.listdir(f"{googlepath}stock_data")]      
            #for file in files:
            #print(file)
            ...
            labels = df.price_x.pct_change(days).map(lambda x: int(x > pct_change/100.0))
            #df = ((df -
    ...
    def train(self):
        ...
        joblib.dump(clf, f'models/clf.pkl')
